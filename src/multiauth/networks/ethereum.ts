...
export function toChainID(
  id: ChainId | ChainIdParams | string | number
): ChainId {
  ...
  const params =
    typeof id === 'object'
      ...
      : {
          namespace: 'eip155',
          reference:
            typeof id === 'number' ? id.toString() : CHAIN_IDS[id] || id,
        };
  return new ChainId(params);
}

/** @internal */
export async function getEIP1193Account(
  provider: EIP1193Provider
): Promise<string | null> {
  try {
    const accounts = await provider.request<Array<string>>({
...
/** @internal */
export async function getWeb3Account(
...
export async function getWeb3ProviderState(
  ...
  const [account, chainID] = await Promise.all([
    ...
    params.chainID ??
      web3ProviderRequest<string>(provider, { method: 'eth_chainId' }),
  ]);
  return { account, chainID: toChainID(chainID) };
}
...
export async function getEthereumProviderState<
  Key extends NetworkProvider<'ethereum'>
>(
  providerKey: Key,

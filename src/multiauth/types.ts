...
export type ConnectorConfigDefaults = DisplayDefaults & {
  ...
  getProvider<Key extends ProviderKey>(
    key: Key,
    params?: unknown
  ): Promise<ProviderTypes[Key]>;
  params?: unknown;
};
...
export type PartialNetworkConfig<Key extends NetworkKey = NetworkKey> =
  | Key
  | (Partial<DisplayDefaults> & {
      key: Key;
      connectors?: Array<PartialConnectorConfig>;
    });

export type NetworkConfig<Key extends NetworkKey = NetworkKey> =
  DisplayDefaults & {
    key: Key;
...
export type AuthenticatedState<Key extends ProviderKey = ProviderKey> =
  NetworkState<Key> & {
    account: string;
  };

...
export function getDefaultNetworkConfig<Key extends NetworkKey>(
  key: Key
): NetworkConfig<Key> {
...
export function getNetworkConfig<Key extends NetworkKey>(
  ...
  const connectors = getConnectorsConfig(
    network.key,
    network.connectors ?? defaults.connectors
  );
  return { ...defaults, ...network, connectors };
}

export function getNetworksConfig(
  providers: Array<PartialNetworkConfig>
): Array<NetworkConfig> {

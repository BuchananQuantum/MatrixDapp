...
import localManifest from '@content/marketplace/manifest.json';
...
export function addTagToSlug(slug: string, tag?: string) {
  return tag ? slug.replace('/marketplace', `/marketplace/tag/${tag}`) : slug;
}

export async function fetchRawMarketPlace(marketplace: string, tag: string) {
  return await getRawFileFromRepo(`${CONTENT_PATH}${marketplace}`, tag);
}
...
export async function fetchDocsManifest(tag: string) {
  ...
  const res = await getRawFileFromRepo(
    `${CONTENT_PATH}/marketplace/manifest.json`,
    tag
  );
  return JSON.parse(res);
}

export function getRawAsset(marketplace: string, tag: string) {
  return getRawAssetFromRepo(`${ASSETS_PATH}${marketplace}`, tag);
}

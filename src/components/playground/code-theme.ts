...
export const getCodeThemeColors = () => {
  ...
  const stringColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('string')
  );
  const punctuationColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('punctuation')
  );
  const numberColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('number')
  );
  const textColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('text')
  );
  const selectorColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('selector')
  );
  const commentColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('comment')
  );
  const classnameColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('tag')
  );
  const attrColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('attr-name')
  );
  const functionColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
    style.types.includes('function')
  );
  const primitiveColor = codeTheme.styles.find((style: { types: string | string[]; }) =>
